...
<tr>
    {{-- رقم الهوية: من users وإن لم يوجد خذه من teacher_profiles --}}
    <td>{{ $u->national_id ?? $u->teacherProfile?->national_id ?? '—' }}</td>

    {{-- الاسم --}}
    <td>{{ $u->name }}</td>

    {{-- المواد الدراسية / التخصص --}}
    <td>{{ $u->teacherProfile?->specialty ?? '—' }}</td>

    {{-- الحالة --}}
    <td>
        @switch($u->status)
            @case('active')   <span class="status status-active">مفعل</span> @break
            @case('pending')  <span class="status status-pending">بانتظار</span> @break
            @case('inactive') <span class="status status-inactive">غير مفعل</span> @break
    ...
    {{-- الإجراءات --}}
    <td>
        <a class="action-btn-table view-btn" href="{{ route('user_admin.teachers.show', $u->id) }}">
            <i class="fas fa-eye"></i> عرض
        </a>
        <a class="action-btn-table edit-btn" href="{{ route('user_admin.teachers.edit', $u->id) }}">
            <i class="fas fa-edit"></i> تعديل
        </a>
        <form action="{{ route('user_admin.teachers.delete', $u->id) }}" method="POST" style="display:inline"
              onsubmit="return confirm('حذف هذا المعلم؟')">
            @csrf @method('DELETE')
