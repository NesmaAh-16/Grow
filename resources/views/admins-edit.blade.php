  ...
  <main class="main-content">
    <header class="main-header">
      <div class="header-title">
        <h1>تعديل إداري</h1>
        <p class="subtitle">حدّث بيانات الإداري. اترك كلمة المرور فارغة إذا لا تريد تغييرها.</p>
      </div>
    ...
    <section style="max-width:720px;">
      <form method="POST" action="{{ route('admin.admins.update', $user->id) }}" class="card-form" style="display:grid;gap:12px">
        @csrf @method('PUT')
        ...
        <div>
          ...
          <select name="status" class="input" required>
            ...
            <option value="active"   {{ $st==='active'?'selected':'' }}>فعال</option>
            <option value="inactive" {{ $st==='inactive'?'selected':'' }}>غير فعال</option>
            <option value="blocked"  {{ $st==='blocked'?'selected':'' }}>محظور</option>
            <option value="pending"  {{ $st==='pending'?'selected':'' }}>معلّق</option>
          </select>
        </div>

        <div>
          <label>كلمة المرور الجديدة (اختياري)</label>
          <input type="password" name="password" class="input" placeholder="اتركها فارغة للإبقاء على الحالية">
        </div>

        <div style="display:flex;gap:8px">
          <button class="btn btn-primary" type="submit"><i class="fas fa-save"></i> تحديث</button>
