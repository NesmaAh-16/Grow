...
<head>
  ...
  <style>
    ...
    .btn{display:inline-block;background:#2f54eb;color:#fff;text-decoration:none;border:none;border-radius:10px;padding:10px 14px}
    .w-100{width:100%}
  </style>
</head>
<body>
        ...
        <div class="assignments-grid">
          @foreach ($assignments as $a)
            <div class="ass-card">
              ...
              <ul class="ass-meta">
                @if($a->published_at) <li>تاريخ النشر: {{ \Carbon\Carbon::parse($a->published_at)->format('Y-m-d H:i') }}</li> @endif
                @if($a->due_at)       <li>موعد التسليم: {{ \Carbon\Carbon::parse($a->due_at)->format('Y-m-d H:i') }}</li> @endif
                <li>الحالة:
              ...
              <a class="btn w-100" href="{{ route('student.assignments.show', $a->id) }}">عرض وتسليم</a>
            </div>
          @endforeach
        </div>
      @endif
